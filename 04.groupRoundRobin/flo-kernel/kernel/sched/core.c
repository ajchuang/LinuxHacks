...
struct cpumask mask_fg;
struct cpumask mask_bg;
...
SYSCALL_DEFINE2(sched_set_CPUgroup, int, numCPU, int, group)
{
	...
	cpumask_clear(&mask_fg);
	cpumask_clear(&mask_bg);

	for_each_possible_cpu(i) {
		if (i <= cpu_grp.fg_cpu_end)
			cpumask_set_cpu(i, &mask_fg);
		if (i >= cpu_grp.bg_cpu_start)
			cpumask_set_cpu(i, &mask_bg);
	}
